...
Enhanced version with advanced terminal visualization and improved error handling
"""
...
# Terminal plotting libraries - try multiple options for better visualizations
PLOTEXT_AVAILABLE = False
TERMGRAPH_AVAILABLE = False
ASCIICHARTPY_AVAILABLE = False
TERMINALPLOT_AVAILABLE = False

# Try importing plotext for CLI charts
try:
...
# Try importing termgraph for bar charts and histograms
try:
    import termgraph
...
# Try importing asciichartpy for line charts
try:
    import asciichartpy
    ASCIICHARTPY_AVAILABLE = True
except ImportError:
    ASCIICHARTPY_AVAILABLE = False

# Try importing terminalplot for simple line plots
try:
    import terminalplot
    TERMINALPLOT_AVAILABLE = True
except ImportError:
    TERMINALPLOT_AVAILABLE = False

# Check if any terminal visualization is available
TERMINAL_VIZ_AVAILABLE = any([PLOTEXT_AVAILABLE, TERMGRAPH_AVAILABLE, 
                             ASCIICHARTPY_AVAILABLE, TERMINALPLOT_AVAILABLE])

if not TERMINAL_VIZ_AVAILABLE:
    print("No terminal visualization libraries available. Install with:")
    print("pip install plotext termgraph asciichartpy")
...
class VisualizationManager:
    ...
    def __init__(self, use_cli: bool = True, output_dir: str = "visualizations"):
        ...
        """
        self.use_cli = use_cli and (RICH_AVAILABLE or TERMINAL_VIZ_AVAILABLE)
    ...
    def display_gene_counts(self, metadata: pd.DataFrame, title: str = "Sample Distribution"):
        ...
        # Use termgraph for bar chart visualization if available
        if TERMGRAPH_AVAILABLE:
            genotype_counts = metadata['genotype'].value_counts().to_dict()
            
            # Format data for termgraph
            labels = list(genotype_counts.keys())
            data = []
            data.append(list(genotype_counts.values()))
            
            # Set configuration options
            args = {
                'filename': '',
                'title': 'Sample Distribution by Genotype',
                'width': min(50, self.terminal_width - 10),
                'format': '{:<5.0f}',
                'suffix': '',
                'no_labels': False,
                'color': None,
            ...
            print("\nSample Distribution by Genotype:")
            termgraph.bargraph(labels=labels, data=data, args=args)
            
            # Show another graph for brain regions
            region_counts = metadata['region'].value_counts().to_dict()
            labels = list(region_counts.keys())
            data = []
            data.append(list(region_counts.values()))
            
            args['title'] = 'Sample Distribution by Brain Region'
            print("\nSample Distribution by Brain Region:")
            termgraph.bargraph(labels=labels, data=data, args=args)
        
        # Fallback to plotext if termgraph is not available
            ...
            region_counts = metadata['region'].value_counts().to_dict()
            plt_cli.clf()
            plt_cli.bar(list(region_counts.keys()), list(region_counts.values()), color="green")
            plt_cli.title("Sample Distribution by Brain Region")
            plt_cli.show()
    ...
    def plot_differential_expression(self, deg_results: pd.DataFrame, 
        ...
        if self.use_cli:
            # Try using plotext for terminal volcano plot
            if PLOTEXT_AVAILABLE:
                plt_cli.clf()
                
                # Simplify for terminal display - use subset of genes
                sample_size = min(500, len(deg_results))
                indices = np.random.choice(len(deg_results), sample_size, replace=False)
                sampled_data = deg_results.iloc[indices]
                
                x_values = sampled_data['log2FoldChange'].values
                y_values = -np.log10(sampled_data['pvalue'].values)
                
                # Color points based on significance and fold change
                colors = []
                for i in range(sample_size):
                    if sampled_data['padj'].iloc[i] < 0.05:
                        if sampled_data['log2FoldChange'].iloc[i] > 1:
                            colors.append("red")
                        elif sampled_data['log2FoldChange'].iloc[i] < -1:
                            colors.append("blue")
                        else:
                ...
                plt_cli.scatter(x_values, y_values, marker="dot", s=5, color=colors)
                plt_cli.title("Volcano Plot: 5xFAD vs Control")
                plt_cli.xlabel("Log2 Fold Change")
                plt_cli.ylabel("-Log10 P-value")
                plt_cli.xlim(-5, 5)
                plt_cli.ylim(0, 10)
                plt_cli.plot_size(self.terminal_width-10, self.terminal_height-5)
                plt_cli.show()
                
            # Try using terminalplot as fallback for simplified plot
            elif TERMINALPLOT_AVAILABLE:
                print("\nVolcano Plot (simplified):")
                
                # Sample a small subset for terminal display
                sample_size = min(100, len(deg_results))
                indices = np.random.choice(len(deg_results), sample_size, replace=False)
                sampled_data = deg_results.iloc[indices]
                ...
                # Scale values to fit terminal size
                terminalplot.plot(x_values, y_values, 
                                  terminal_width=min(80, self.terminal_width),
                                  terminal_height=min(20, self.terminal_height-5))
    ...
    def plot_pathway_enrichment(self, pathway_results: pd.DataFrame,
        ...
        # Display in terminal if possible
        if self.use_cli:
            # Option 1: Use termgraph for terminal bar graph if available
            if TERMGRAPH_AVAILABLE:
                # Format data for termgraph
                # Take just top 7 pathways to avoid cluttering terminal
                display_pathways = top_pathways.head(7)
                
                # For simplicity, show shortened pathway names
                labels = []
                for name in display_pathways['pathway_name']:
                    if len(name) > 25:
                ...
                values = -np.log10(display_pathways['adjusted_p_value']).tolist()
                data = []
                data.append(values)
            ...
            # Option 2: Use plotext as fallback
            elif PLOTEXT_AVAILABLE:
                plt_cli.clf()
                
                # Take top 7 pathways for clarity
                display_pathways = top_pathways.head(7).copy()
                ...
                log_pvalues = -np.log10(display_pathways['adjusted_p_value']).tolist()
                
                # Use a horizontal bar chart
                plt_cli.clc()  # Clear color data
                plt_cli.bar(log_pvalues, short_names, orientation="horizontal", color="blue")
                plt_cli.title("Top Enriched Pathways")
                plt_cli.xlabel("-Log10 Adjusted P-value")
                plt_cli.plot_size(None, len(short_names) + 5)  # Adjust height based on number of pathways
                plt_cli.show()
            
            # Option 3: Use Rich table as another fallback
            elif RICH_AVAILABLE:
                ...
                for _, row in top_pathways.iterrows():
                    # Create a visual representation of significance
                    sig_level = -np.log10(row['adjusted_p_value'])
                    sig_bar = "█" * min(int(sig_level * 2), 20)
                    
                    table.add_row(
                        row['pathway_name'], 
                        str(row['gene_count']),
                        f"{row['adjusted_p_value']:.2e}",
                        Text(sig_bar, style="blue")
                    )
    ...
    def plot_compound_ranking(self, compounds: List[Dict[str, Any]], 
        ...
        # Display in terminal if possible
        if self.use_cli:
            # Option 1: Use termgraph for horizontal bar chart
            if TERMGRAPH_AVAILABLE:
                # Format data for termgraph - limit to top 8 for display
                display_compounds = sorted_compounds[:8]
                labels = [name for name, _ in display_compounds]
                values = [score for _, score in display_compounds]
                data = []
                data.append(values)
                ...
                args = {
                    'filename': '',
                    'title': 'Compound Ranking for Alzheimer\'s Treatment',
                    'width': min(40, self.terminal_width - 20),
                    'format': '{:<5.1f}',
                    'suffix': '',
                    'no_labels': False,
                    'color': None,
            ...
            # Option 2: Use plotext as fallback
                ...
                # Limit to top 8 compounds for clarity
                display_compounds = sorted_compounds[:8]
                names = [name for name, _ in display_compounds]
                values = [score for _, score in display_compounds]
                
                plt_cli.bar(values, names, orientation="horizontal", color="green")
                plt_cli.title("Compound Ranking")
                plt_cli.xlabel("Efficacy Score")
                plt_cli.xlim(0, 10)
                plt_cli.plot_size(None, len(names) + 5)  # Adjust height based on number of compounds
                plt_cli.show()
            
            # Option 3: Use asciichartpy for a different visualization style
            elif ASCIICHARTPY_AVAILABLE:
                print("\nCompound Efficacy Scores:")
                
                # Sort values for visual appeal in line chart
                sorted_values = sorted(scores_values)
                chart = asciichartpy.plot(sorted_values, {'height': 10, 'format': '{:8.1f}'})
                print(chart)
            
            # Option 4: Use Rich table as another fallback
            elif RICH_AVAILABLE:
                ...
                for i, (name, score) in enumerate(sorted_compounds, 1):
                    # Create a visual bar representation of score
                    visual_bar = "█" * int(score)
                    
                    table.add_row(
                        str(i), 
                        name, 
                        f"{score:.1f}",
                        Text(visual_bar, style="green")
                    )
                
                console.print(table)
    
    def display_heatmap(self, data: np.ndarray, row_labels: List[str], 
                      col_labels: List[str], title: str = "Expression Heatmap",
        ...
        Args:
            ...
            output_file: File to save the plot
        """
        # Create matplotlib heatmap and save to file
        plt.figure(figsize=(max(8, len(col_labels) * 0.4), max(6, len(row_labels) * 0.4)))
        
        # Use a seaborn-like colormap
        cmap = plt.cm.RdBu_r
        
        im = plt.imshow(data, cmap=cmap, aspect='auto')
        plt.colorbar(im, fraction=0.046, pad=0.04)
        
        # Add row and column labels if there aren't too many
        if len(row_labels) <= 20 and len(col_labels) <= 20:
        ...
        # Display in terminal if possible
        if self.use_cli:
            # Option 1: Use plotext for terminal heatmap
            if PLOTEXT_AVAILABLE:
                try:
                    # For terminal display, limit the size
                    max_rows = min(20, len(row_labels))
                    max_cols = min(20, len(col_labels))
                    
                    # Sample data if too large
                    if len(row_labels) > max_rows or len(col_labels) > max_cols:
                        # Subsample rows and columns
                        row_indices = np.linspace(0, len(row_labels)-1, max_rows, dtype=int)
                        col_indices = np.linspace(0, len(col_labels)-1, max_cols, dtype=int)
                        
                        display_data = data[row_indices, :][:, col_indices]
                        display_row_labels = [row_labels[i] for i in row_indices]
                        display_col_labels = [col_labels[i] for i in col_indices]
                    else:
                        display_data = data
                    ...
                    plt_cli.clf()
                    plt_cli.colorless()
                    plt_cli.heatmap(display_data, 
                                   index=False,
                                   xside=max(10, min(self.terminal_width-10, len(display_data[0])*3)),
                                   yside=max(5, min(self.terminal_height-6, len(display_data)*1)))
                    plt_cli.title(title)
                    plt_cli.show()
                    
                    print("Note: Terminal heatmap is a simplified representation")
                    print(f"Full heatmap saved to {os.path.join(self.output_dir, output_file)}")
                except Exception as e:
                    logger.warning(f"Error displaying terminal heatmap: {str(e)}")
            
            # Option 2: Use Rich table for very small heatmaps
            elif RICH_AVAILABLE and len(row_labels) <= 10 and len(col_labels) <= 10:
                table = Table(title=title)
                ...
                for i, row_label in enumerate(row_labels):
                    ...
                    for j in range(len(col_labels)):
                        ...
                        intensity = min(abs(value) * 5, 5)
                        bars = "█" * int(intensity)
                        
                        row_data.append(Text(f"{value:.2f} {bars}", style=color))
                    
                    table.add_row(*row_data)
                
                console.print(table)
            else:
                print(f"Heatmap saved to {os.path.join(self.output_dir, output_file)}")
                print("(Terminal visualization not available for this heatmap)")
    
    def display_timeline(self, events: List[Dict[str, Any]], title: str = "Pipeline Timeline"):
        """
        Display a timeline of pipeline events
        
        Args:
            events: List of event dictionaries with 'time', 'event', and 'status'
            title: Timeline title
        """
        if not self.use_cli or not events:
            return
        
        # Option 1: Use Rich for timeline display
        if RICH_AVAILABLE:
            table = Table(title=title)
            ...
            for event in events:
                ...
                status_style = "green" if event.get('status') == 'completed' else \
                               "yellow" if event.get('status') == 'in_progress' else \
                               "red" if event.get('status') == 'failed' else "white"
                
                # Create status text with appropriate styling
                status_text = Text(event.get('status', 'unknown'), style=status_style)
        ...
        # Option 2: Use termgraph for a timeline-like chart
        elif TERMGRAPH_AVAILABLE:
            # Format events for termgraph
            labels = [event.get('event', '') for event in events]
            
            # Use duration as bar length if available, otherwise use 1
            has_duration = all('duration_seconds' in event for event in events)
            
            if has_duration:
                data = []
                data.append([event.get('duration_seconds', 0.1) for event in events])
                ...
                # Just print events if no duration data
                print(f"\n{title}:")
                for i, event in enumerate(events, 1):
                    status = event.get('status', '')
                    status_marker = '✓' if status == 'completed' else '⋯' if status == 'in_progress' else '✗'
                    print(f"{i}. {status_marker} {event.get('time', '')} - {event.get('event', '')}")
        
        # Option 3: Just print the events
        else:
            print(f"\n{title}:")
            for event in events:
                status = event.get('status', '')
                status_marker = '[DONE]' if status == 'completed' else '[RUNNING]' if status == 'in_progress' else '[FAILED]'
                print(f"{event.get('time', '')} - {event.get('event', '')} {status_marker} {event.get('duration', '')}")
    
    def display_network_graph(self, nodes: List[Dict[str, Any]], 
                             edges: List[Dict[str, Any]],
                             title: str = "Interaction Network", 
                             output_file: str = "network.png"):
        """
        Create a network graph visualization
        
        Args:
            nodes: List of node dictionaries with at least 'id' and 'name'
            edges: List of edge dictionaries with 'source', 'target', and optional 'weight'
            title: Graph title
            output_file: File to save the plot
        """
        # Check if networkx is available for graph visualization
        try:
            ...
            nx.draw_networkx_nodes(G, pos, 
                                  node_size=[300 * G.nodes[node].get('size', 1.0) for node in G.nodes],
                                  node_color=[G.nodes[node].get('color', 'skyblue') for node in G.nodes],
                                  alpha=0.8)
            ...
            # Draw labels if we don't have too many nodes
            if len(G.nodes) <= 50:
            ...
            # Save to file
            plt.savefig(os.path.join(self.output_dir, output_file), dpi=300)
            logger.info(f"Network graph saved to {os.path.join(self.output_dir, output_file)}")
            
            # Terminal visualization is limited for networks, display message
            if self.use_cli:
                if RICH_AVAILABLE:
                    console.print(f"[green]Network graph with {len(nodes)} nodes and {len(edges)} edges saved to {output_file}[/green]")
                    console.print("[yellow]Terminal cannot display complex network graphs[/yellow]")
                else:
                    print(f"Network graph with {len(nodes)} nodes and {len(edges)} edges saved to {output_file}")
                    print("Terminal cannot display complex network graphs")
                
                # For small networks, we can show edge list in a table
                if RICH_AVAILABLE and len(edges) <= 15:
                    table = Table(title=f"{title} - Edge List")
                    ...
                    for edge in sorted_edges[:15]:  # Limit to top 15
                        source_name = next((node['name'] for node in nodes if node['id'] == edge['source']), edge['source'])
                        target_name = next((node['name'] for node in nodes if node['id'] == edge['target']), edge['target'])
            ...
            print(f"NetworkX not available for network visualization. Graph saved as node-edge list.")
            
            # Save as edge list
            with open(os.path.join(self.output_dir, f"{output_file}.txt"), 'w') as f:
                f.write(f"{title}\n\n")
                f.write("Nodes:\n")
                for node in nodes:
                    f.write(f"  {node['id']}: {node.get('name', '-')}\n")
                ...
                for edge in edges:
                    ...
                    f.write(f"  {source} -- {target} ({weight:.2f}) {label}\n")
    ...
    def display_pipeline_summary(self, results: Dict[str, Any]):
        ...
        # Option 1: Use Rich for a detailed summary table
        if RICH_AVAILABLE:
        ...
        # Option 2: Use termgraph for a visual summary if available
        elif TERMGRAPH_AVAILABLE:
            # Create a summary of key statistics
            labels = ["Samples", "Genes", "DE Genes", "Pathways", "Compounds"]
            values = [
                results.get('sample_count', 0),
                results.get('gene_count', 0),
                len(results.get('upregulated_genes', [])) + len(results.get('downregulated_genes', [])),
                len(results.get('top_pathways', [])),
                len(results.get('compounds', []))
            ]
            ...
            args = {
                'filename': '',
                'title': 'Alzheimer\'s Drug Discovery Pipeline Summary',
                'width': min(50, self.terminal_width - 15),
                'format': '{:<5.0f}',
                'suffix': '',
                'no_labels': False,
                'color': None,
            ...
            # Print top compounds separately
            if results.get('compounds'):
                print("\nTop Potential Compounds:")
                for i, compound in enumerate(results.get('compounds', [])[:5], 1):
                    print(f"{i}. {compound.get('name', 'Unknown')}")
        
        # Option 3: Use plotext as fallback
        elif PLOTEXT_AVAILABLE:
            plt_cli.clf()
            ...
            values = [
                results.get('sample_count', 0),
                results.get('gene_count', 0) / 100,  # Scale down genes count for better visualization
                len(results.get('upregulated_genes', [])) + len(results.get('downregulated_genes', [])),
                len(results.get('top_pathways', [])),
                len(results.get('compounds', []))
            ]
            
            plt_cli.bar(labels, values, orientation="vertical", color="cyan")
            plt_cli.title("Pipeline Summary")
            plt_cli.show()
        ...
        # Option 4: Simple text summary as last resort
        else:
            print("\n=== Alzheimer's Drug Discovery Pipeline Summary ===")
            print(f"Samples Analyzed: {results.get('sample_count', 'N/A')}")
            print(f"Genes Analyzed: {results.get('gene_count', 'N/A')}")
            print(f"Differentially Expressed Genes: Up: {len(results.get('upregulated_genes', []))}, Down: {len(results.get('downregulated_genes', []))}")
            print(f"Enriched Pathways: {len(results.get('top_pathways', []))}")
            print(f"Compounds Identified: {len(results.get('compounds', []))}")
            print(f"Combination Therapies: {len(results.get('combination_therapies', {}))}")
...
class AlzheimerDrugDiscovery:
    ...
    def __init__(self, 
        ...
        # Initialize event tracking for timeline visualization
        ...
        self._add_pipeline_event("Pipeline Initialization", "completed")
        
        # Display rich header if available
        if RICH_AVAILABLE:
            console.print(Panel.fit(
                "[bold cyan]Alzheimer's Drug Discovery Pipeline[/bold cyan]\n"
                "[green]Combining transcriptomics and AI for novel therapeutics with enhanced visualization[/green]",
                border_style="blue"
            ))
    
    def _add_pipeline_event(self, event_name: str, status: str, details: str = ""):
        """
        Add an event to the pipeline timeline
        
        Args:
            event_name: Name of the event
            status: Status of the event (completed, in_progress, failed)
    ...
    def load_data(self) -> bool:
        ...
        """
        try:
            self._add_pipeline_event("Data Loading", "in_progress")
            
            if self.use_mock_data:
                logger.info("Using mock transcriptomic data")
                self._generate_mock_expression_data()
                self._add_pipeline_event("Data Loading", "completed", "Using mock data")
                return True
            ...
            self._add_pipeline_event("Data Loading", "completed", 
                                   f"{self.expression_data.shape[0]} genes, {self.expression_data.shape[1]-1} samples")
            return True
            
        except Exception as e:
            logger.error(f"Erro ao carregar dados: {str(e)}")
            self._add_pipeline_event("Data Loading", "failed", str(e))
            
            if self.use_mock_data:
                logger.info("Falling back to mock data after error")
                self._generate_mock_expression_data()
                self._add_pipeline_event("Data Loading (Fallback)", "completed", "Using mock data")
                return True
            return False
    ...
    def run_differential_expression(self, save_results: bool = True) -> bool:
        ...
        self._add_pipeline_event("Differential Expression Analysis", "in_progress")
        ...
        try:
            ...
            self._add_pipeline_event("Differential Expression Analysis", "completed", 
                                   f"{len(self.upregulated_genes)} up-regulated genes, {len(self.downregulated_genes)} down-regulated genes")
            
            # Create a heatmap for the top differentially expressed genes if visualization is enabled
            if self.viz_manager and self.expression_data is not None:
                try:
                    # Get top 20 up and down regulated genes
                    top_up = self.upregulated_genes[:10]
                    top_down = self.downregulated_genes[:10]
                    top_genes = top_up + top_down
                    
                    # Extract expression data for these genes
                    if 'gene_id' in self.expression_data.columns:
                        gene_expr = self.expression_data[self.expression_data['gene_id'].isin(top_genes)]
                        gene_expr = gene_expr.set_index('gene_id')
                    else:
                        gene_expr = self.expression_data.loc[top_genes]
                    
                    # Create a simple heatmap matrix
                    heatmap_data = gene_expr.values
                    ...
                    # Create heatmap
                    self.viz_manager.display_heatmap(
                        heatmap_data_scaled, 
                        gene_expr.index.tolist(), 
                        gene_expr.columns.tolist(), 
                        "Top Differentially Expressed Genes"
                    )
                    
                except Exception as e:
                    logger.warning(f"Could not create heatmap: {str(e)}")
            
            return True
            
        except Exception as e:
            logger.error(f"Erro na análise de expressão diferencial: {str(e)}")
            self._add_pipeline_event("Differential Expression Analysis", "failed", str(e))
            return False
    ...
    def run_pathway_analysis(self, save_results: bool = True) -> bool:
        ...
        self._add_pipeline_event("Pathway Analysis", "in_progress")
        ...
        try:
            ...
            self._add_pipeline_event("Pathway Analysis", "completed", 
                                   f"Top pathways: {', '.join(self.top_pathways[:3])}")
            return True
            
        except Exception as e:
            logger.error(f"Erro na análise de vias: {str(e)}")
            self._add_pipeline_event("Pathway Analysis", "failed", str(e))
            return False
    ...
    def identify_potential_compounds(self) -> bool:
        ...
        self._add_pipeline_event("Compound Identification", "in_progress")
        ...
        self._add_pipeline_event("Compound Identification", "completed", 
                               f"Identified {len(self.compounds)} unique compounds")
    ...
    def simulate_drug_gene_interactions(self, top_n: int = 5) -> bool:
        ...
        self._add_pipeline_event("Drug-Gene Interaction Analysis", "in_progress")
        
        if not self.compounds:
            logger.error("Nenhum composto identificado para simular interações")
            self._add_pipeline_event("Drug-Gene Interaction Analysis", "failed", "No compounds identified")
            return False
        ...
        if not target_genes:
            ...
            self._add_pipeline_event("Drug-Gene Interaction Analysis", "failed", "No target genes found")
            return False
        ...
        # Create network visualization if possible
        if self.viz_manager and len(self.compound_interactions) > 0:
            try:
                # Create simplified interaction network for visualization
                nodes = []
                edges = []
                
                # Add compound nodes
                for i, compound_name in enumerate(self.compound_interactions.keys()):
                    nodes.append({
                        'id': f"comp_{i}",
                        'name': compound_name,
                        'type': 'compound',
                ...
                # Add gene nodes and extract interactions
                gene_ids = set()
                for text in self.compound_interactions.values():
                    # Simple regex to find gene names in the text (simple pattern for demonstration)
                    gene_matches = re.findall(r'\b([A-Z0-9]+(?:\d[A-Z])?)\b', text)
                    for gene in gene_matches:
                        if len(gene) >= 3 and gene not in ['DNA', 'RNA', 'APP', 'NMDA', 'ACHE', 'PKC', 'ROS']:
                            gene_ids.add(gene)
                ...
                # Create some example edges based on text analysis
                compound_node_ids = [node['id'] for node in nodes if node['type'] == 'compound']
                gene_node_map = {node['name']: node['id'] for node in nodes if node['type'] == 'gene'}
                
                for comp_idx, (compound_name, interaction_text) in enumerate(self.compound_interactions.items()):
                    comp_node_id = compound_node_ids[comp_idx]
                    
                    # Check if each gene appears in this compound's interaction text
                    for gene, gene_node_id in gene_node_map.items():
                        if gene in interaction_text:
                            # Determine interaction type based on keywords
                            if "inhibit" in interaction_text.lower() or "antagonist" in interaction_text.lower():
                                interaction_type = "inhibits"
                                weight = 2.0
                            elif "activat" in interaction_text.lower() or "stimulat" in interaction_text.lower():
                                interaction_type = "activates"
                                weight = 2.0
                            elif "modulat" in interaction_text.lower() or "regulat" in interaction_text.lower():
                ...
                self.viz_manager.display_network_graph(nodes, edges, "Compound-Gene Interaction Network")
                
            except Exception as e:
                logger.warning(f"Could not create interaction network visualization: {str(e)}")
        
        self._add_pipeline_event("Drug-Gene Interaction Analysis", "completed", 
                               f"Analyzed {len(self.compound_interactions)} compounds")
    ...
    def rank_compounds(self) -> bool:
        ...
        self._add_pipeline_event("Compound Ranking", "in_progress")
        
        if not self.compound_interactions:
            logger.error("Nenhuma interação composto-gene para ranquear")
            self._add_pipeline_event("Compound Ranking", "failed", "No compound interactions to rank")
            return False
        ...
        if self.compound_ranking:
            ...
            self._add_pipeline_event("Compound Ranking", "completed", 
                                   f"Ranked {len(self.compound_scores)} compounds")
            return True
        else:
            logger.error("Falha ao ranquear compostos")
            self._add_pipeline_event("Compound Ranking", "failed", "Failed to generate ranking")
            return False
    ...
    def simulate_combination_therapy(self, top_n: int = 3) -> bool:
        ...
        self._add_pipeline_event("Combination Therapy Analysis", "in_progress")
        
        if not self.compounds or len(self.compounds) < 2:
            logger.error("Número insuficiente de compostos para simular combinações")
            self._add_pipeline_event("Combination Therapy Analysis", "failed", 
                                   "Insufficient number of compounds")
            return False
        ...
        self._add_pipeline_event("Combination Therapy Analysis", "completed", 
                               f"Analyzed {len(self.combination_therapies)} combinations")
    ...
    def validate_with_literature(self, top_n: int = 3) -> bool:
        ...
        self._add_pipeline_event("Literature Validation", "in_progress")
        
        if not self.compounds:
            logger.error("Nenhum composto para validar")
            self._add_pipeline_event("Literature Validation", "failed", "No compounds to validate")
            return False
        ...
        self._add_pipeline_event("Literature Validation", "completed", 
                               f"Validated {len(self.literature_validation)} compounds")
        
        # Display summarized literature validation table if rich is available
        if RICH_AVAILABLE and self.literature_validation:
            table = Table(title="Literature Evidence Summary")
            table.add_column("Compound", style="cyan")
            table.add_column("Evidence Strength", style="green", justify="center")
            table.add_column("Clinical Trials", style="yellow", justify="center")
            table.add_column("Animal Studies", style="blue", justify="center")
            
            for compound, lit_text in self.literature_validation.items():
                # Simple text analysis to determine evidence levels
                evidence_strength = "MODERATE"
                if "STRONG" in lit_text:
                    evidence_strength = "STRONG"
                elif "PRELIMINARY" in lit_text:
                    evidence_strength = "PRELIMINARY"
                
                has_clinical = "Phase" in lit_text or "clinical trial" in lit_text.lower()
                has_animal = "mouse" in lit_text.lower() or "model" in lit_text.lower() or "animal" in lit_text.lower()
                
                clinical_status = "✓" if has_clinical else "✗"
                animal_status = "✓" if has_animal else "✗"
                
                table.add_row(compound, evidence_strength, clinical_status, animal_status)
    ...
    def generate_final_report(self) -> bool:
        ...
        if self.final_report:
            ...
            self._add_pipeline_event("Final Report Generation", "completed")
            return True
        else:
            logger.error("Falha ao gerar relatório final")
            self._add_pipeline_event("Final Report Generation", "failed", "Failed to generate report")
            return False
    
    def display_timeline(self) -> None:
        """Display the timeline of pipeline execution"""
        if self.viz_manager and self.pipeline_events:
            self.viz_manager.display_timeline(self.pipeline_events, "Pipeline Execution Timeline")
...
def main():
    """Main function to run the pipeline with safe imports and scope handling"""
    # First, explicitly (re)import all required libraries and components inside main()
    # This ensures they're available in the local scope
    import argparse
    import os
    import sys
    import logging
    
    # Create flags for library availability
    RICH_AVAILABLE = False
    PLOTEXT_AVAILABLE = False
    TERMGRAPH_AVAILABLE = False
    ASCIICHARTPY_AVAILABLE = False
    ...
    # Display options
    display_group = parser.add_argument_group('display', 'Display Options')
    display_group.add_argument("--terminal-plots", action="store_true", help="Use terminal-based plots when possible")
    display_group.add_argument("--install-missing", action="store_true", help="Automatically install missing visualization libraries")
    
    args = parser.parse_args()
    
    # Attempt to install missing libraries if requested
    if args.install_missing:
        try:
            import subprocess
            print("Checking for missing visualization libraries...")
            
            # Check and install rich
            if not RICH_AVAILABLE:
                print("Installing rich library...")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "rich"])
                print("Rich installed successfully.")
                
                # Try importing rich again
                try:
                    import rich
                    from rich.console import Console
                    from rich.progress import Progress, TextColumn, BarColumn, TimeElapsedColumn
            ...
            # Check and install termgraph
            if not TERMGRAPH_AVAILABLE:
                print("Installing termgraph library...")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "termgraph"])
                print("Termgraph installed successfully.")
                
                # Try importing termgraph again
                try:
                    import termgraph
                    TERMGRAPH_AVAILABLE = True
                except ImportError:
            ...
            # Check and install asciichartpy
            if not ASCIICHARTPY_AVAILABLE:
                print("Installing asciichartpy library...")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "asciichartpy"])
                print("Asciichartpy installed successfully.")
                
                # Try importing asciichartpy again
                try:
                    import asciichartpy
                    ASCIICHARTPY_AVAILABLE = True
                except ImportError:
            ...
            # Update terminal visualization availability flag
            TERMINAL_VIZ_AVAILABLE = any([PLOTEXT_AVAILABLE, TERMGRAPH_AVAILABLE, 
                                        ASCIICHARTPY_AVAILABLE, TERMINALPLOT_AVAILABLE])
    ...
    if RICH_AVAILABLE:
        try:
            console.print(Panel.fit(
                "[bold cyan]Alzheimer's Drug Discovery Pipeline[/bold cyan]\n"
                "[green]Enhanced version with Terminal Visualization and improved error handling[/green]",
                border_style="blue"
            ))
            
            # Show available visualization libraries
            libs_table = Table(title="Terminal Visualization Capabilities")
            libs_table.add_column("Library", style="cyan")
            libs_table.add_column("Status", style="green")
            libs_table.add_column("Features", style="yellow")
            
            libs_table.add_row("Rich", "✓ Available" if RICH_AVAILABLE else "✗ Not Available", "Tables, Progress Bars, Panels")
            libs_table.add_row("Plotext", "✓ Available" if PLOTEXT_AVAILABLE else "✗ Not Available", "Line Charts, Bar Charts, Scatter Plots, Heatmaps")
            libs_table.add_row("Termgraph", "✓ Available" if TERMGRAPH_AVAILABLE else "✗ Not Available", "Bar Charts, Calendar Heatmaps")
            libs_table.add_row("Asciichartpy", "✓ Available" if ASCIICHARTPY_AVAILABLE else "✗ Not Available", "Line Charts")
            
            console.print(libs_table)
        except Exception as e:
            print(f"Error displaying rich formatting: {str(e)}")
            print("\nAlzheimer's Drug Discovery Pipeline")
            print("Enhanced version with Terminal Visualization and improved error handling")
            print("\nAvailable visualization libraries:")
            print(f"- Rich: Partially Available (error encountered)")
            print(f"- Plotext: {'Available' if PLOTEXT_AVAILABLE else 'Not Available'}")
            print(f"- Termgraph: {'Available' if TERMGRAPH_AVAILABLE else 'Not Available'}")
            print(f"- Asciichartpy: {'Available' if ASCIICHARTPY_AVAILABLE else 'Not Available'}")
    else:
        print("\nAlzheimer's Drug Discovery Pipeline")
        print("Enhanced version with Terminal Visualization and improved error handling")
        print("\nAvailable visualization libraries:")
        print(f"- Rich: {'Available' if RICH_AVAILABLE else 'Not Available'}")
        print(f"- Plotext: {'Available' if PLOTEXT_AVAILABLE else 'Not Available'}")
        print(f"- Termgraph: {'Available' if TERMGRAPH_AVAILABLE else 'Not Available'}")
        print(f"- Asciichartpy: {'Available' if ASCIICHARTPY_AVAILABLE else 'Not Available'}")
    
    # Configure the pipeline with passed visualization flags
    from alzheimer_drug_discovery import AlzheimerDrugDiscovery
...
# To fix the entire module, you should modify all global imports and variables.
# Here's a suggestion for how to structure your main program file:
